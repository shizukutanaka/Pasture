...
| **IMP-24** | 出力長予測でルーティング/コスト見積り（ヒューリスティック版採用、proxyモデル版deferred） | 1,7 | SSJF 2404.08509 | ✅(ADR-135) |
...
- **IMP-24 出力長予測:** 軽量proxyモデル（SSJF）は依存増＝**非採用（deferred）**。代替の
  **ヒューリスティック見積り**（プロンプト長×タスク種別比率）を **✅ 採用済み（ADR-135）**：
  `estimate_output_tokens` が入力＋予測出力トークンを budget/spike 判定に供給。proxyモデル版は今も deferred。
...
| 24 | ヒューリスティック版 ✅採用（ADR-135）／proxyモデル版は deferred | ✅ |
