...
| **A5. Config-file layer is test-only** | ⏸️ **Open, named.** `from_str_with_defaults`/`apply` are called only from `#[cfg(test)]` code while `SPEC.md` documents the file layer normatively. Decide: wire it into `Config::default().with_env()` or strike the spec sentence. Next iteration. |
