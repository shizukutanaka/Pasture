...
| **E13. HEAD only worked on one route, and elsewhere it sent a body** | ✅ **FIXED (ADR-280).** RFC 9110 §9.1 requires HEAD wherever GET is supported; only `/health` did it. `HEAD /v1/models`, `/v1/stats`, `/v1/history`, `/metrics`, `/dashboard` answered 405 with an `Allow` list that omitted HEAD — and that 405 carried a body, which §9.3.2 forbids and which desynchronises a keep-alive client. One switch fixes all routes: HEAD is dispatched as GET and every writer goes through a single `write_typed` that drops the content when `head_only`. The refactor of four writers into one moved no bytes: 11 of 13 wire captures byte-identical, the other two being the intended HEAD change. |
