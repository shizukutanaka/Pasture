...
- **(W5) `proxy.rs` is ~4k lines.** The single-file HTTP/routing/dispatch module
  ...
  *Narrowed, not closed (ADR-275):* response builders now live in
  `src/response.rs`; `proxy.rs` is 4,198 lines, most of it `impl Proxy`.
