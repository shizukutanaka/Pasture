...
| **S5. Privacy detection has the same blind spot** | ❗ **Open, higher stakes.** `privacy::normalize_for_detection` folds full-width *digits* but strips nothing, so the invisible-character trick that defeated the guard plausibly defeats PII value detection too — and that guards **I2** (sensitive content must not leave the machine), not just an opt-in flag. Next security item. |
