...
| **S6. Pseudonymizer spans still read raw text** | ⏸️ **Open, bounded.** `*_spans` index original bytes, so an obfuscated value would not be masked. It does **not** reopen the leak — such prompts are now forced local by S5 — but it would matter under the explicit `PASTURE_ALLOW_SENSITIVE_CLOUD` opt-out. Needs an offset-mapping pass, not a filter. |
