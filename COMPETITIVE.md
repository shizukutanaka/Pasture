...
| **S7. Residual encodings** | ⏸️ **Open, named honestly.** Only **one** decode level is attempted, so compositional/nested encodings still evade, as do leetspeak and other in-word substitutions, Morse and fictional ciphers the model learned but this scanner doesn't, mixed-language payloads, multi-turn staged attacks, and novel phrasing. `guard.rs`'s "Known limitations" enumerates exactly this. |
