...
All commands:               pasture help\n",
    ...
    ("doctor.ollama.version.ok", "     Ollama {version} reports logprobs: cascade, label and calibrate can score confidence"),
    ("doctor.ollama.version.old", "[--] Ollama {version} predates 0.12.11: no logprobs, so the cascade falls back to a text heuristic"),
    ("doctor.ollama.version.stale", "[!!] Ollama {version} predates 0.12.11: PASTURE_CASCADE is on but cannot score confidence, so it falls back to a text heuristic"),
    ("doctor.ollama.version.old.fix", "     upgrade Ollama (https://ollama.com) to score confidence on this machine"),
...
すべてのコマンド:        pasture help\n",
    ...
    ("doctor.ollama.version.ok", "     Ollama {version} は logprobs を返します: cascade / label / calibrate で確信度を測れます"),
    ("doctor.ollama.version.old", "[--] Ollama {version} は 0.12.11 より古く logprobs を返しません。cascade はテキスト判定にフォールバックします"),
    ("doctor.ollama.version.stale", "[!!] Ollama {version} は 0.12.11 より古いため、PASTURE_CASCADE を有効にしても確信度を測れず、テキスト判定にフォールバックします"),
    ("doctor.ollama.version.old.fix", "     Ollama を更新すると（https://ollama.com）このマシンで確信度を測れます"),
